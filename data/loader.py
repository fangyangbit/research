...
class DataLoader(object):
    ...
    def __getitem__(self, key):
        ...
        #将实体向量添加到batch
        subj_vec=get_float_tensor(batch[9],batch_size)
        obj_vec=get_float_tensor(batch[10],batch_size)
        subj_pvec=get_pvec_tensor(batch[11],batch_size)
        obj_pvec=get_pvec_tensor(batch[12],batch_size)
...
def get_float_tensor(tokens_list, batch_size):
   ...
   # print(tokens_list)
    ...
    return tokens
def get_pvec_tensor(tokens_list, batch_size):
    """ Convert list of list of tokens to a padded LongTensor. """
    dim=300
    token_len = max(len(x) for x in tokens_list)
    #print("token_len:{}".format(token_len))
