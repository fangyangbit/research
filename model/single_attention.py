...
class AffectiveAttention(nn.Module):
    ...
    def forward(self, drug_vec, patient_vec, lengths=None):
       ...
       # print(attention_input.size())
        c = 1-self.sigmoid(self.gate(features) * attention_input)
        #z:complementary information
        z=(self.gate(features) * attention_input)*c
        # step 4: attend over the features
        #attentions:传回来的是score
        #representations, attentions = self.attention(z, lengths)
